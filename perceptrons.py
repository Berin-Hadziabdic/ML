...
perceptron_network =  np.random.uniform(-0.05,0.05,size=(785,10)) #785 weights X 10 perceptrons
...
def train():
    ...
    while accuracy  < 0.855:
        ...
        for test_data in test_images:
            biased_test_data = np.append([1], test_data) # Add bias to test data
            percp_class = np.argmax(np.dot( biased_test_data,perceptron_network)) #This number contains the highest firing perceptron index which corresponds to the classified #.
...
train()
plt.ylabel("Epoch Training/Test Accuracy Graph")
